...
int read_nii(const char *path, sift3d_image *const im) {
        return nii_error_message();
}

int write_nii(const char *path, const sift3d_image *const im) {
        return nii_error_message();
}
...
/* Write a sift3d_image to the specified path, in NIFTI format.
